...
SetDynaCDSPreference(PDESKTROUBLESHOOT This)
{
    ...
    {
        _ltow(This->lCDSPref, szCDSPref, 16);
        DPRINT1("Saving reg value: %ws\n", szCDSPref);
        RegSetValueExW(hKey, L"DynaSettingsChange", 0, REG_SZ, (LPBYTE)&szCDSPref, sizeof(WCHAR));
        RegCloseKey(hKey);
    }
}
...
GetDynaCDSPreference(PDESKTROUBLESHOOT This)
{
    ...
    /* Default preference (apply new settings dynamically) */
    ULONG lCDSPref = 1;
    ...
    /* Check if we got a string and that it is not empty */
    if (lResult != ERROR_SUCCESS || dwDataBuff <= sizeof(UNICODE_NULL))
    {  

        dwDataBuff = sizeof(szCDSData);
        DPRINT1("HKCU failed, trying HKLM! A!\n");
    ...
    if (!(lResult != ERROR_SUCCESS || dwDataBuff <= sizeof(UNICODE_NULL)))
    {
        ...
        lCDSPref = _wtol(szCDSData);
	}

    DPRINT1("DEBUG: _wtoi: %d, first char: %d, result: %d, buffer %d\n",lCDSPref, szCDSData[0], lResult, dwDataBuff);
...
{
    LPWSTR szDeskDisplayDevice = NULL;
    LPWSTR szDeviceKeyPos = NULL;
    SIZE_T lenDeviceKey = 0;

    szDeskDisplayDevice = QueryDeskCplString(This->pdtobj,
                                                      RegisterClipboardFormatW(
                                                      DESK_EXT_DISPLAYKEY));

    lenDeviceKey = lstrlenW(szDeskDisplayDevice);
    This->szDeviceKey = (LPWSTR)malloc(sizeof(WCHAR)*lenDeviceKey+sizeof(UNICODE_NULL));
    
    if (This->szDeviceKey == NULL)
        return;
    
    CharUpperW(szDeskDisplayDevice);
    
    szDeviceKeyPos = wcsstr(szDeskDisplayDevice, L"\\SYSTEM");
