...
// FindAC97Device
/////////////////////////////////////////////////////////////////////////////////
// This function stores the device info and device data of the "ac97" driver.
...
// "ac97". The information is stored in pspRequest.
//
// For simplicity we search for the service name "ac97".
...
void DisplayPropertySheet (HWND hWnd)
{
    ...
    {
        iRes = LoadString(ghInstance, IDS_AC97_WARNING_MUST_INSTALL_DRIVER, wszFormat, _ARRAYSIZE(wszFormat));
        if (iRes >0)
        {
    ...
    {
        iRes = LoadString(ghInstance, IDS_AC97_WARNING_PROPERTYPAGE_DLL_COULD_NOT_LOAD, wszFormat, _ARRAYSIZE(wszFormat));
        if (iRes >0)
        {
            MessageBox (hWnd, wszFormat, AppletName, MB_ICONSTOP | MB_OK);
        }
    ...
    {
        iRes = LoadString(ghInstance, IDS_AC97_WARNING_PROPERTYPAGEPROVIDER_FAILURE, wszFormat, _ARRAYSIZE(wszFormat));
        if (iRes >0)
        {
            MessageBox (hWnd, wszFormat, AppletName, MB_ICONSTOP | MB_OK);
        }
    ...
    if (PropertySheet (&psh) < 0)
    {
        ...
        //
        iRes = LoadString(ghInstance, IDS_AC97_WARNING_REINSTALL_DRIVER, wszFormat, _ARRAYSIZE(wszFormat));
        if (iRes >0)
        {
            MessageBox (hWnd, wszFormat, AppletName, MB_ICONSTOP | MB_OK);
        }
