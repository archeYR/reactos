...
void
W32Prof_Test_GL11TexturedCube(const ProfilerConfig* cfg)
{
    HDC hdc;
    HGLRC rc;
    RECT r;
    ...
    if (!hRender)
    {
        ResultsPrint(TEXT("OpenGL 1.1 Textured: failed to create render child window"));
        return;
    }
    ...
    {
        ResultsPrint(TEXT("OpenGL 1.1 Textured: Choose/SetPixelFormat failed"));
    ...
    if (!rc)
    {
        ResultsPrint(TEXT("OpenGL 1.1 Textured: wglCreateContext failed: %lu"), GetLastError());
        ReleaseDC(hRender, hdc);
        DestroyWindow(hRender);
        return;
    }
    ...
    {
        ResultsPrint(TEXT("OpenGL 1.1 Textured: wglMakeCurrent failed: %lu"), GetLastError());
        wglDeleteContext(rc);
        ReleaseDC(hRender, hdc);
        DestroyWindow(hRender);
        return;
    ...
    {
        ResultsPrint(TEXT("OpenGL 1.1 Textured: failed to load embedded TGA"));
    ...
    while (1)
    {
        ...
        i++;
        W32Prof_FpsMaybeReport(cfg, &fps, i, qf.QuadPart, TEXT("OpenGL 1.1 Textured Cube"));
    }

    QueryPerformanceCounter(&q1);

    ResultsPrint(TEXT("OpenGL 1.1 Textured Cube: %lu frames in %.3f ms (%.2f fps)"),
                 (ULONG)i,
