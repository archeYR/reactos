...
void
W32Prof_Test_D3D9TexturedCubeFullscreen(const ProfilerConfig* cfg)
{
    IDirect3D9* d3d;
    ...
    if (!hRender)
    {
        ResultsPrint(TEXT("D3D9 Textured FS: failed to create fullscreen window"));
        return;
    }

    if (!W32Prof_LoadLogoTestTgaFromResource(GetModuleHandle(NULL), &img))
    {
        ResultsPrint(TEXT("D3D9 Textured FS: failed to load embedded TGA"));
    ...
    if (!d3d)
    {
        ResultsPrint(TEXT("D3D9 Textured FS: Direct3DCreate9 failed"));
    ...
    if (FAILED(hr) || !dev)
    {
        ResultsPrint(TEXT("D3D9 Textured FS: CreateDevice failed: 0x%08lx"), (ULONG)hr);
    ...
    {
        ResultsPrint(TEXT("D3D9 Textured FS: texture upload failed"));
    ...
    if (FAILED(hr) || !vb)
    {
        ResultsPrint(TEXT("D3D9 Textured FS: CreateVertexBuffer failed: 0x%08lx"), (ULONG)hr);
        IDirect3DTexture9_Release(tex);
        IDirect3DDevice9_Release(dev);
        IDirect3D9_Release(d3d);
        W32Prof_ImageFree(&img);
    ...
    ResultsPrint(TEXT("D3D9 Textured Cube (Fullscreen): %lu frames in %.3f ms (%.2f fps)"),
                 (ULONG)i,
