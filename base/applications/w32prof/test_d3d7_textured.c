...
/*
 * D3D7 textured spinning cube test (windowed).
 */
...
CreateTextureFromRgba(LPDIRECTDRAW7 dd, const W32PROF_IMAGE_RGBA* img, LPDIRECTDRAWSURFACE7* outTex, BOOL* outIs565)
{
    ...
    *outTex = NULL;
    ...
    if (FAILED(hr) || !tex)
    {
        ...
        if (FAILED(hr) || !tex)
        {
            ResultsPrint(TEXT("D3D7 Textured: Create texture surface failed: 0x%08lx"), (ULONG)hr);
            return FALSE;
        }
    ...
    /* Upload */
    {
        ...
        if (FAILED(hr) || !lock.lpSurface)
        {
            ...
            ResultsPrint(TEXT("D3D7 Textured: Lock texture failed: 0x%08lx"), (ULONG)hr);
            return FALSE;
        }
...
void
W32Prof_Test_D3D7TexturedCube(const ProfilerConfig* cfg)
{
    HRESULT hr;
    RECT rc;
    int w, h;
    ...
    if (!hRender)
    {
        ResultsPrint(TEXT("D3D7 Textured: failed to create render child window"));
        return;
    }

    if (!W32Prof_LoadLogoTestTgaFromResource(GetModuleHandle(NULL), &img))
    {
        ResultsPrint(TEXT("D3D7 Textured: failed to load embedded TGA"));
    ...
    if (FAILED(hr) || !dd)
    {
        ResultsPrint(TEXT("D3D7 Textured: DirectDrawCreateEx failed: 0x%08lx"), (ULONG)hr);
    ...
    if (FAILED(hr))
    {
        ResultsPrint(TEXT("D3D7 Textured: SetCooperativeLevel failed: 0x%08lx"), (ULONG)hr);
...

    {
        ...
        {
            ResultsPrint(TEXT("D3D7 Textured: Create primary surface failed: 0x%08lx"), (ULONG)hr);
            IDirectDraw7_Release(dd);
            W32Prof_ImageFree(&img);
            DestroyWindow(hRender);
            return;
        ...
        if (FAILED(hr) || !surf3d)
        {
            ResultsPrint(TEXT("D3D7 Textured: Create 3D surface failed: 0x%08lx"), (ULONG)hr);
    ...
    if (FAILED(hr) || !d3d)
    {
        ResultsPrint(TEXT("D3D7 Textured: QI IID_IDirect3D7 failed: 0x%08lx"), (ULONG)hr);
    ...
    if (FAILED(hr) || !dev)
    {
        ResultsPrint(TEXT("D3D7 Textured: CreateDevice failed: 0x%08lx"), (ULONG)hr);
    ...
    /* Z buffer */
    {
        ...
        if (!zctx.Found)
        {
            ResultsPrint(TEXT("D3D7 Textured: no Z buffer formats; disabling Z"));
        }
        else
        {
            ...
            if (FAILED(hr) || !zsurf)
            {
                ResultsPrint(TEXT("D3D7 Textured: Create Z surface failed: 0x%08lx"), (ULONG)hr);
                zsurf = NULL;
            }
            else
            {
    ...
    ResultsPrint(TEXT("D3D7 Textured Cube: %lu frames in %.3f ms (%.2f fps)"),
                 (ULONG)i,
